...
namespace denizdikbiyik_CET322_HW5.Areas.Identity.Pages.Account
{
    ...
    public class ExternalLoginModel : PageModel
    {
        ...
        public async Task<IActionResult> OnGetCallbackAsync(string returnUrl = null, string remoteError = null)
        {
            ...
            if (remoteError != null)
            {
                ErrorMessage = $"Dış sağlayıcıdan hata: {remoteError}";
                return RedirectToPage("./Login", new {ReturnUrl = returnUrl });
            }
            var info = await _signInManager.GetExternalLoginInfoAsync();
            if (info == null)
            {
                ErrorMessage = "Dış giriş bilgisi yükleme hatası.";
                return RedirectToPage("./Login", new { ReturnUrl = returnUrl });
            }
            ...
            if (result.Succeeded)
            {
                _logger.LogInformation("{Name} giriş yaptı {LoginProvider} şu sağlayıcı ile.", info.Principal.Identity.Name, info.LoginProvider);
