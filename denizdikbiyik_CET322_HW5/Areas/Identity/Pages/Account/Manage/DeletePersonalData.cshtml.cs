    ...
    public class DeletePersonalDataModel : PageModel
    {
        ...
        public async Task<IActionResult> OnPostAsync()
        {
            ...
            if (RequirePassword)
            {
                ...
                {
                    ModelState.AddModelError(string.Empty, "Şifre doğru değil.");
                    return Page();
                }
            }
            ...
            if (!result.Succeeded)
            {
                throw new InvalidOperationException($"Kişiyi silerken bilinmeyen bir hata oluştu '{userId}'.");
            }

            await _signInManager.SignOutAsync();

            _logger.LogInformation("Şu kişi '{UserId}' kendini sildi.", userId);
